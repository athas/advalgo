...
class Metaheuristic:
    ...
    def update(self):
        ...
        for p in self.solution.solution:
            pygame.draw.circle(self.window, (0, 0, 0), scale(p), self.pointSize)
        for i in range(0, len(self.solution.solution)):
            pygame.draw.line(self.window, (0,0,255), pt(i%len(self.algorithm.solution.solution)), pt((i+1)%len(self.algorithm.solution.solution)))
...
def main():
    ...
    if algorithm.solutionval < bestval:
        ...
        h.close()
    print "Current: "+str(algorithm.solutionval)
